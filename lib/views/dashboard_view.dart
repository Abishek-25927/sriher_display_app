...
const _kApiKey  = '933cdb13cb54e31e694f82bf7f75f0144a9495036db0243b85dd855be53c06f2';
...
class _DashboardData {
  ...
  factory _DashboardData.fromJson(Map<String, dynamic> json) {
    ...
    if (vids != null && vids.isNotEmpty) {
      vidUrl = '$_kBaseUrl${vids.first['file_name']}';
    } else if (json['vinci'] != null && (json['vinci'] as List).isNotEmpty) {
      vidUrl = '$_kBaseUrl${(json['vinci'] as List).first['file_name']}';
    }
...
class _DashboardViewState extends State<DashboardView>
  ...
  bool _hasEverPlayed = false; // once user taps play for first time
  bool _showControls = true;  // controls overlay visibility
  String? _videoError;        // non-null when init failed
  String? _videoUrl;          // saved for retry
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (body['status'] == 'Success') {
          ...
          _initVideo(d.videoUrl);
          return;
        }
      }
      setState(() { _loading = false; _error = 'Server error'; });
    } catch (ex) {
      setState(() { _loading = false; _error = ex.toString(); });
    }
  }
  ...
  void _initVideo(String? url) {
    ...
    try {
      ...
      _player.setVolume(0.0); // Start muted to avoid autoplay blocks on some platforms, or as per user preference
      ...
      // Trigger 1: duration becomes non-zero (video decoded)
      _durationSub = _player.stream.duration.listen((dur) {
        if (dur > Duration.zero) markReady();
      });

      // Trigger 2: buffering state change (player started loading)
      _bufferingSub = _player.stream.buffering.listen((buffering) {
        // When buffering first starts and then stops it means player is ready
        if (!buffering) markReady();
      });

      // Trigger 3: Fallback timeout — show video widget after 5 seconds
      // regardless, so it never stays stuck on the loading spinner.
      _videoReadyTimer = Timer(const Duration(seconds: 5), markReady);
    } catch (err) {
  ...
  Widget _topCards(_DashboardData d) {
    return Padding(
      ...
      child: Row(children: [
        _menuCard('DEVICE DETAILS',   Icons.tv,                  '${d.totalDevice}',
            const LinearGradient(colors: [Color(0xFFFF9800), Color(0xFFFFB74D)]), _cardAnimations[0]),
        _menuCard('TEMPLATE DETAILS', Icons.dashboard_customize, '${d.totTemp}',
            const LinearGradient(colors: [Color(0xFF1976D2), Color(0xFF64B5F6)]), _cardAnimations[1]),
        _menuCard('LOCATION',         Icons.location_on,         '${d.totLocation}',
            const LinearGradient(colors: [Color(0xFF388E3C), Color(0xFF81C784)]), _cardAnimations[2]),
        _menuCard('FILES',            Icons.insert_drive_file,   '${d.imgFile}',
            const LinearGradient(colors: [Color(0xFF546E7A), Color(0xFF90A4AE)]), _cardAnimations[3]),
      ]),
    );
  }

  Widget _menuCard(String title, IconData icon, String count,
      LinearGradient grad, Animation<double> anim) {
    return Expanded(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(child: ScaleTransition(
            ...
            child: _card(child: _videoPanel(d.videoUrl), height: 420),
          )),
        ],
      ),
    );
  ...
  Widget _slideshow(List<String> urls) {
    if (urls.isEmpty) {
      return const Center(child: Text('No images', style: TextStyle(color: Colors.grey)));
    }
  ...
  Widget _videoPanel(String? url) {
    ...
    if (_videoError != null) {
      return _videoPanelPlaceholder(
        ...
        showRetry: true,
        onRetry: () => _initVideo(_videoUrl),
      );
    }

    // Loading / initialising — show dark spinner, NOT a blurry video frame
    if (!_videoReady) {
      return Container(
              ...
              builder: (context, durationSnap) {
                ...
                return StreamBuilder<double>(
                  ...
                  builder: (context, volumeSnap) {
                    ...
                    return GestureDetector(
                      // Tap on controls area toggles controls; first tap plays
                      onTap: () {
                        if (!_hasEverPlayed) {
                          _player.play();
                          setState(() {
                      ...
                      child: ClipRRect(
                        ...
                        child: Stack(children: [
                          ...
                          // ── Before first play: solid black + big Play icon ──
                          // This covers the blurry initial decoded frame.
                          if (!_hasEverPlayed)
                            Positioned.fill(
                              child: Container(
                                color: Colors.black,
                          ...
                          // ── Gradient overlay (when playing & controls visible) ──
                          ...
                          // ── Big centered Play/Pause (only while playing & controls showing) ──
                          if (_hasEverPlayed && _showControls)
                            Center(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Expanded(flex: 1, child: Column(children: [
                ...
                child: Padding(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const Text('Progress Track',
                          style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13, letterSpacing: 0.5)),
                      const SizedBox(height: 4),
                      const Divider(color: Colors.white12),
                      _categoryRow(Icons.tv,                  Colors.orange, 'Devices',    d.deviceList.length),
                      _categoryRow(Icons.dashboard_customize, Colors.blue,   'Templates',  d.tempList.length),
                      _categoryRow(Icons.location_on,         Colors.green,  'Locations',  d.locationList.length),
                      _categoryRow(Icons.view_quilt,          Colors.purple, 'Over Views', d.overview.length),
                    ],
                  ),
                ),
              ),
            ...
            const Align(
              ...
              child: Padding(
                ...
                child: Text('© 2026 SRIHER Display',
                    style: TextStyle(color: Colors.white54, fontSize: 11)),
              ),
            ),
          ])),
          ...
          // Right: dynamic table
          Expanded(flex: 4, child: ScaleTransition(
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            ...
            borderRadius: BorderRadius.circular(8),
            border: Border.all(
              color: isSelected ? Colors.tealAccent.withValues(alpha: 0.5) : Colors.white10,
            ),
          ),
          child: Row(
            children: [
              Icon(icon, color: isSelected ? Colors.tealAccent : Colors.white70, size: 18),
        ...
        child: Column(children: [
          Row(children: [
            ...
            Icon(Icons.arrow_forward_ios,
                color: sel ? color : Colors.grey.shade400, size: 11),
          ]),
  ...
  List<String> get _headers {
    switch (_selectedCategory) {
      case 'Devices':    return ['Device Name', 'Device Code', 'Model'];
      case 'Templates':  return ['Template Name', 'Duration'];
      case 'Locations':  return ['Location', 'Floor', 'Sub Location'];
      case 'Over Views': return ['Device', 'Schedule', 'Template', 'Duration', 'Location'];
      default:           return [];
    }
  }
    ...
    if (rows.isEmpty) {
      return SizedBox(
        ...
        child: Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
          ...
          const SizedBox(height: 10),
          Text('No data for $_selectedCategory',
