...
class _SpecificRangesViewState extends State<SpecificRangesView> {
  ...
  Future<void> _fetchScheduleNames() async {
    ...
    try {
      final response = await http.post(Uri.parse('$_baseUrl/schedulerange_scheduleNamesview'),
  ...
  Future<void> _fetchTemplates() async {
    ...
    try {
      final response = await http.post(Uri.parse('$_baseUrl/new_templateview'),
          headers: {"Content-Type": "application/json"},
          body: jsonEncode({"api_key": _apiKey}));
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
  ...
  Future<void> _fetchTemplateFiles(int tempId) async {
    ...
    try {
      final response = await http.post(Uri.parse('$_baseUrl/schedulerange_temptableview'),
          headers: {"Content-Type": "application/json"},
  ...
  Future<void> _fetchOffDates() async {
    ...
    try {
      final response = await http.post(Uri.parse('$_baseUrl/schedulerange_getOffDatesview'),
          headers: {"Content-Type": "application/json"},
          body: jsonEncode({
            "api_key": _apiKey,
  ...
  Future<void> _handleInsertSlots() async {
    if (selectedScheduleId == null || selectedTemplateId == null || _fromDateController.text.isEmpty || _toDateController.text.isEmpty) {
      _showSnackBar("Please complete all required fields");
      return;
    }

    // Filter selected slots
    List<int> selectedIndices = slotSelection.entries.where((e) => e.value).map((e) => e.key).toList();
    ...
    for (int index in selectedIndices) {
      ...
      try {
        final response = await http.post(Uri.parse('$_baseUrl/schedulerange_insertSlotsview'),
            headers: {"Content-Type": "application/json"},
    ...
    return Column(
      children: [
        Container(
          ...
          child: const Text("SLOTS", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13)),
        ),
        Container(
          height: 380,
          width: double.infinity,
                        ...
                        children: [
                          Transform.scale(
                            ...
                            child: Checkbox(
                              ...
                              onChanged: (v) => setState(() => slotSelection[index] = v!),
                              visualDensity: VisualDensity.compact,
      ...
      children: [
        ...
        Container(
          ...
          child: Row(
            ...
            children: const [
              Expanded(flex: 1, child: Text("ORDER", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10))),
              Expanded(flex: 3, child: Text("FILE NAME", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10))),
              Expanded(flex: 2, child: Text("FILE", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10))),
              Expanded(flex: 2, child: Text("DURATION", style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 10))),
            ],
          ),
        ),
        Container(
          ...
          child: templateFiles.isEmpty
              ? const Center(child: Text("No data available in table", style: TextStyle(fontSize: 11, color: Colors.grey)))
              : ListView.builder(
                    ...
                    return Container(
                      ...
                      child: Row(
                        children: [
                          ...
                          Expanded(flex: 3, child: Text(file['user_filename'] ?? file['file_name'] ?? '-', style: const TextStyle(fontSize: 10), overflow: TextOverflow.ellipsis)),
                          Expanded(
                            flex: 2,
                          ...
                          Expanded(flex: 2, child: Text("${file['duration'] ?? '30'}s", style: const TextStyle(fontSize: 10))),
                        ],
                      ),
                    );
                  },
      ...
      child: Row(
        ...
        children: [
          Flexible(
            child: Row(
              children: [
                const Text("Show ", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
                SizedBox(
                  width: 70, height: 35,
                  child: DropdownButtonFormField<String>(
                    value: entriesValue,
                    items: ["10", "25"].map((e) => DropdownMenuItem(value: e, child: Text(e, style: const TextStyle(fontSize: 11)))).toList(),
                ...
                const Text(" entries", style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold)),
              ],
            ),
          ),
  ...
  Widget _buildPaginationFooter() {
    return Padding(
      ...
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          Text("Showing 1 to ${templateFiles.length} of ${templateFiles.length} entries", style: const TextStyle(fontSize: 10, fontWeight: FontWeight.bold)),
          Row(
            children: [
      ...
      children: [
        ...
        SizedBox(
          ...
          child: TextFormField(
            ...
            onTap: () async {
              DateTime? p = await showDatePicker(context: context, initialDate: DateTime.now(), firstDate: DateTime(2000), lastDate: DateTime(2101));
              if (p != null) {
                setState(() => controller.text = DateFormat('yyyy-MM-dd').format(p));
  ...
  Widget _buildDropdown({required String hint, required int? value, required List<dynamic> items, required ValueChanged<int?> onChanged}) {
    return SizedBox(
      height: 40,
